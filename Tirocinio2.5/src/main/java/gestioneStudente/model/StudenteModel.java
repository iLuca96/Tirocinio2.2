...
/**
 * 
 * StudenteModel è una classe con 9 metodi, questa classe si occupa di gestire varie operazioni di inserimento, modifica e cancellazione delle tuple della tabella studente. 
 *
 */
public class StudenteModel {
	...
	/**
	 * Il metodo doModifyCurriculum, viene utilizzato per poter modificare il link di dov'è situato il file.
	 * @param studente tipo Studente, variabile che ci da accesso a tutti i metodi set e get.
	 * @return result tipo int, restituisce il risultato della query di UPDATE.
	 * @throws SQLException eccezione che viene lanciata quando viene rilevato un errore nell'esecuzione di una query.
	 */
	 ...
	 * @return customers tipo Collection di Studente, Array che conterrà tutte le informazioni sullo Studente
	 * @throws SQLException eccezione che viene lanciata quando viene rilevato un errore nell'esecuzione di una query.
	 */
	 ...
	 * @throws SQLException eccezione che viene lanciata quando viene rilevato un errore nell'esecuzione di una query.
	 */
	public synchronized Studente loginStudente(String email_username,String psw) throws SQLException {
		Connection connection = null;
		PreparedStatement preparedStatement = null;

		Studente bean = new Studente();

		String selectSQL = "SELECT * FROM " + StudenteModel.TABLE_NAME + " WHERE (Email = ? OR username = ?) AND psw = ?";
